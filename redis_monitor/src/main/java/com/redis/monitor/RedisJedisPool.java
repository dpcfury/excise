...
public class RedisJedisPool {
	...
	public static void initRedisJedisPool(List<RedisServer> rsList) {
		...
		for (RedisServer rs : rsList) {
			BasicRedisCacheServer server = new RedisCacheServer(rs.getHost(),
					rs.getPort(), rs.getMaxActive(), rs.getMaxIdle(),
					rs.getMaxWait(), rs.isTestOnBorrow());
			...
			for (RedisServer slRs : slaveRs) {
				BasicRedisCacheServer readServer = new RedisCacheServer(
						slRs.getHost(), slRs.getPort(), slRs.getMaxActive(),
						slRs.getMaxIdle(), slRs.getMaxWait(),
						slRs.isTestOnBorrow());
	...
	public static void removeRedisServer(String uuid) {
		if (uuid != null && !uuid.equals("") && map.containsKey(uuid))
			map.remove(uuid);
	}

	public static BasicRedisCacheServer getRedisCacheServer(String uuid) {
		return map.get(uuid) == null ? null : map.get(uuid)
				.getBasicRedisCacheServer();
	}
