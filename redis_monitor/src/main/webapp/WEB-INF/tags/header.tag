...
<script>
	...
	function flushAll(){
		...
		$.getJSON('<%=request.getContextPath()%>/flushall.htm' , function(data){
	...
	function flushDB(){
		...
		$.getJSON('<%=request.getContextPath()%>/flushDb.htm' , function(data){
			if(data.statu.toLowerCase() == 'ok'){
				message('flushDB操作成功') ;
			} else {
				message('flushDB操作失败') ;
		...
		<div class="container">
			...
			<div class="nav-collapse">
				<ul class="nav">
					<li class="dropdown"><a href="#" class="dropdown-toggle"
						...
						<ul class="dropdown-menu">
						   <c:forEach var="rs" items="${redisServerList }">
						     <li class=""><a href="<%=request.getContextPath()%>/index.htm?uuid=${rs.uuid }">${rs.description }</a></li>
						   </c:forEach>
					...
					<li><a href="<%=request.getContextPath()%>/keys.htm">keys</a></li>
					<li><a href="<%=request.getContextPath()%>/config/toConfigDetail.htm">redis-config.xml</a></li>
					<li><a href="<%=request.getContextPath()%>/server/redisServer.htm">redis节点管理</a></li>
					<li><a href="<%=request.getContextPath()%>/monitor.htm">redis实时监控</a></li>
				</ul>
