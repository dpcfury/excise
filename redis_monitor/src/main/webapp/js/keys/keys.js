$(function(){
	...
	function searchListener(e){
		...
		var url = contextPath+'/keys/getByPattern.htm';
...
function getValue(key){
	...
	var url = contextPath+'/keys/value.htm?key=' + key +'&uuid=' + uuid ;
...
function updateString(){
	...
	$.post(contextPath+'/keys/updateString.htm' , {key:key , value:val} , function(data){
		console.log(data) ;
		if(data.status == 0){
			message('修改成功') ;
		} else {
...
function deleteString(){
	$.post(contextPath+'/keys/deleteString.htm' , {key:key} , function(data){
		console.log(data) ;
		if(data.status == 0){
			message('删除成功') ; 
		} else {
