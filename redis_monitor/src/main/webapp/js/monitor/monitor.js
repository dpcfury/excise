$(function(){
	...
	function start(){
		$.getJSON(contextPath+'/monitor/start.htm?uuid=' + uuid , function(data){
			if(data.status == 0){
				refresh() ;
			} else {
				alert("开始失败") ;
	...
	function stop(){
		$.getJSON(contextPath+'/monitor/stop.htm?uuid=' + uuid , function(data){
			if(data.status == 0){
	...
	function refresh(){
		...
		$.getJSON(contextPath+'/monitor/data.htm?uuid=' + uuid , function(data){
